                    ...
                    <div class="card-body box-shadow">
                        <div class="table-responsive" id="table-display">
                            <table class="table table-striped table-bordered zero-configuration">
                                ...
                                <tbody>
                                    @foreach ($social_mediasdata as $key => $value)
                                        <tr>
                                            <td>{{ ++$key }}</td>
                                            ...
                                            <td>
                                                @if ($value->is_available == 1)
                                                    <button class="btn btn-sm btn-success rounded-4" tooltip="Active"
                                                        onclick="StatusUpdate('{{ $value->id }}','2','{{ route('admin.social_media.status') }}')">
                                                        <i class="fa-sharp fa-solid fa-check"></i>
                                                    </button>
                                                @else
                                                    <button class="btn btn-sm btn-danger rounded-4" tooltip="Inactive"
                                                        onclick="StatusUpdate('{{ $value->id }}','1','{{ route('admin.social_media.status') }}')">
                                                        <i class="fa-sharp fa-solid fa-xmark"></i>
                                                    </button>
                                                @endif
                                            </td>
                                            <td>
                                                <a href="{{ route('admin.social_media.edit', $value->id) }}" tooltip="Edit"
                                                ...
                                                <button class="btn btn-sm btn-danger rounded-4" tooltip="Delete"
                                                    onclick="Delete('{{ $value->id }}','{{ route('admin.social_media.delete') }}')">
